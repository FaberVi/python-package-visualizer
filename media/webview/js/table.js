...
window.renderTable = function (filtered) {
  ...
  if (countEl) {
    if (filtered.length === window.allPackages.length) {
      ...
      countEl.innerHTML = `${window.t('result.showing')} <strong>${filtered.length}</strong> ${window.t('result.of')} <strong>${window.allPackages.length}</strong> ${window.t('result.packages')} <span id="btn-clear-filters" class="clear-link">${window.t('result.clear')}</span>`;
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = `
      <tr>
        <td colspan="8" style="padding:40px 20px;text-align:center;color:var(--vscode-descriptionForeground)">
  ...
  const listRows = filtered.map(pkg => {
    ...
    if (pkg.vulnerabilities && pkg.vulnerabilities.length > 0) {
      tagsHtml += ` <span class="tag-cve" title="Has security vulnerabilities">${window.t('tag.cve')}</span>`;
    }
    if (pkg.hasConflict) {
      tagsHtml += ` <span class="tag-conflict" title="Has dependency conflicts">${window.t('tag.conflict')}</span>`;
    }
    if (!pkg.isUsed && pkg.status !== 'not-installed') {
      tagsHtml += ` <span class="tag-unused" title="Not imported in python files">${window.t('tag.unused')}</span>`;
    }
    ...
    if (pkg.specifiedVersion && pkg.installedVersion) {
      ...
      if (m) {
        reqVersion = m[1];
        if (reqVersion !== pkg.installedVersion) {
          hasDrift = true;
        }
      }
    }
    ...
    if (hasDrift) {
      tagsHtml += ` <span class="tag-drift" title="${window.t('tag.driftTitle')}">${window.t('tag.drift')} (${reqVersion})</span>`;
    }

    // Synchronize requirements file action button
    let syncBtnHtml = '';
    if (hasDrift) {
      syncBtnHtml = `<button class="action-btn sync-btn" data-name="${esc(pkg.name)}" data-source="${esc(pkg.source || '')}" title="${window.t('btn.syncTitle')}">${window.t('btn.sync')}</button> `;
    }
...
window.updateBulkBar = function () {
  ...
  if (window.selectedPackages.size > 0) {
    ...
    if (elBulkCount) elBulkCount.textContent = `${window.selectedPackages.size} ${window.t('bulk.selected')}`;
  } else {
